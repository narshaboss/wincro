...
### 2026-03-19: 업데이트 배치의 한글 exe 검출 안정화 (v1.0.158)

- **배치 검증 방식 변경:** 업데이트 후 새 exe를 파일명 하드코딩으로 검사하지 않고, 앱 폴더의 실제 *.exe를 동적으로 찾아 검증/재시작하도록 변경
- **한글 exe/경로 안정화:** 자동/수동 업데이트 배치를 utf-8-sig로 저장해 Windows 배치의 한글 파일명 해석 문제를 완화
- **롤백 오탐 방지:** 기존 설치 exe 이름과 새 배포 exe 이름이 달라도 exe 없음으로 잘못 롤백되지 않도록 수정

---
### 2026-03-19: 업데이트 롤백 exe 이름 불일치 수정 (v1.0.157)

- **자동 업데이트 롤백 수정:** 기존 설치 exe 이름과 새 배포 exe 이름이 달라도, zip 내부에서 실제 exe 파일명을 찾아 검증/재시작하도록 수정
- **자동 업데이트 경로 보강:** [app.py] 배치 스크립트가 새 exe 이름 기준으로 무결성 검사와 재시작을 수행하도록 변경
- **수동 업데이트 경로 보강:** [settings_view.py] 수동 업데이트도 새 exe 존재 확인 후 그 이름으로 실행하도록 변경
- **공통 유틸 확장:** extract_and_find_exe()가 (앱 디렉토리, exe 파일명)을 함께 반환하도록 변경
