...
### 2026-02-08: 조건 이미지 로직 원복 - 발견=점프실행 (v1.0.120)

**변경:** v1.0.113에서 반전된 조건 이미지 로직을 원래대로 복원. "조건 이미지 발견 = 조건 충족 = 점프 실행"이 올바른 동작. 반전 로직으로 인해 조건 이미지가 화면에 있을 때 점프가 영원히 차단되는 문제 발생.

**수정된 파일:**
- `src/player/rule_executor.py` - 조건 이미지 발견 시 condition_met=True(점프실행), 미발견 시 False(대기)
- `src/utils/config.py` - APP_VERSION 1.0.119 → 1.0.120
