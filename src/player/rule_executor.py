...
class RuleExecutor:
    ...
    def _handoff_special_mode_rule(
        ...
        *,
        ...
        self._notify_complete(True, message)
        return True
    ...
    def _notify_complete(self, success: bool, message: str) -> None:
        """Reset HID state before UI callbacks are allowed to start another run."""
        with self._completion_lock:
            if self._completion_notified:
                logger.warning("[InputSafety] duplicate execution completion ignored")
                return
            self._completion_notified = True
        self._end_shared_image_scan()
        if not self._reset_input_state("execution-complete"):
            block_automation_input("RuleExecutor.completion_reset_failed")
            success = False
            message = f"입력 장치 안전 해제 실패: {message}"
        # Never perform another release after the callback. The callback can
        # immediately start the next repeat, and a late release would cancel
        # input generated by that new run.
        self._input_reset_completed = True
        ...
        try:
            ...
            logger.error(f"execution completion callback failed: {exc}")
    ...
    def execute_plan(
        ...
        *,
        ...
        with self._lifecycle_lock:
            if self._state in [
                ExecutionState.RUNNING_INITIAL,
                ExecutionState.MONITORING,
                ExecutionState.PAUSED,
            ]:
                logger.warning("이미 실행 중입니다")
                return False
            previous_thread = self._execution_thread

        # Never wait for the current worker from its own completion callback.
        if previous_thread and previous_thread.is_alive():
            if previous_thread is threading.current_thread():
                logger.warning("현재 실행 콜백 안에서는 새 실행을 바로 시작할 수 없습니다")
                return False
        ...
        with self._lifecycle_lock:
            if self._state in [
                ...
                ExecutionState.MONITORING,
                ExecutionState.PAUSED,
            ] or (self._execution_thread and self._execution_thread.is_alive()):
                logger.warning("다른 실행이 먼저 시작되어 요청을 취소합니다")
                return False
            ...
            try:
                ...
                with self._completion_lock:
                    self._completion_notified = False
                self._input_reset_completed = False
                if not self._reset_input_state("execution-start"):
                    raise RuntimeError("input reset failed before execution start")
                ...
                all_rules_count = (
                    len(self._flatten_rules(plan.initial_rules))
                    + len(self._flatten_rules(plan.monitoring_rules))
                )
                self._progress = ExecutionProgress(
                ...
                # Reserve the running state before Thread.start(). Concurrent
                # play clicks must not create a second worker in this window.
                self._state = ExecutionState.RUNNING_INITIAL
                execution_thread = threading.Thread(
                    target=self._execution_loop,
                    name="wincro-rule-executor",
                    daemon=True,
                )
                ...
                logger.error(f"실행 스레드 시작 실패: {exc}")
                self._execution_thread = None
                self._state = ExecutionState.FAILED
                self._stop_event.set()
                self._pause_event.set()
                block_automation_input("RuleExecutor.start_failed")
                self._reset_input_state("execution-start-failed")
                return False
    ...
    def _mark_stop_requested(self) -> None:
        """Serialize stop state with execution startup."""
        with self._lifecycle_lock:
            self._stop_event.set()
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            self._reset_input_state("execution-stop")
            try:
    ...
    def _execution_loop(self) -> None:
        ...
        try:
            ...
            while current_repeat < total_repeat_count:
                current_repeat += 1
                if current_repeat > 1:
                    if not self._reset_input_state(f"repeat-{current_repeat}-start"):
                        raise RuntimeError("input reset failed at repeat boundary")
                if total_repeat_count > 1:
                ...
                while i < len(all_rules_with_step):
                    ...
                    if getattr(result, "rewind_previous_action", False):
                        ...
                        if rewind_target_rule_id:
                            ...
                            if (
                                ...
                                self._notify_complete(False, f"동작 실패: {message}")
                                return
                    ...
                    if monitoring_jump_index >= 0 or monitoring_jump_rule_id:
                        ...
                        if target_index < 0:
                            ...
                            if self._on_error:
                                self._on_error(message, rule)
                            self._notify_complete(False, message)
                            return
                    ...
                    if not result.success:
                        ...
                        self._notify_complete(False, f"동작 실패: {result.message}")
                        return
            ...
            if not self._stop_event.is_set():
                ...
                self._notify_complete(True, f"자동화 실행 완료: {success_count}/{total_count} 성공")
            ...
            self._end_shared_image_scan()
            if not self._input_reset_completed:
                if not self._reset_input_state("execution-finally"):
                    block_automation_input("RuleExecutor.final_reset_failed")
                self._input_reset_completed = True
